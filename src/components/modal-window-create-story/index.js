...
export default function ModalWindowCreateStory({closeModal}){
  ...
  const sendStory = () => {
    const formData = new FormData();
    formData.append('video', image);  // 'image' - это имя поля в вашем бэкенде
    ...
    <>
      <div className="modal_window">
        ...
        <div className="content">
          <h2>Создай новый сторис</h2>
          {selectedImage && <img className="storyCreateImg" src={selectedImage} alt="new post" />}
          {!selectedImage && <div className="content_drag_and_drop">
          ...
          <br />
          <button onClick={sendStory} className="button">Поделиться сторис</button>
        </div>
    </div>
